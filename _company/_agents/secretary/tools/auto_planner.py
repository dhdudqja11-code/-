...
def main():
    ...
    if total_h <= 0:
        ...
        print(f"\n🚀 [오토 플래너] {total_h}시간 동안 {interval_h}시간마다 캠페인 발행 (제한 모드)")
        print(f"⚠️  종료까지 {total_h}시간 채팅창 점유. Ctrl+C로 중단 가능.")
    print()

    if not os.path.exists(ORCHESTRATOR_PATH):
        print(f"❌ campaign_orchestrator.py를 찾을 수 없어요: {ORCHESTRATOR_PATH}")
        sys.exit(1)
    ...
    # 첫 실행 전 campaign_orchestrator.py가 정상 동작하는지 빠르게 검증
    print("🔍 campaign_orchestrator.py 첫 회차 검증 중 (~30초)...")
    test_proc = subprocess.run([sys.executable, ORCHESTRATOR_PATH], capture_output=True, text=True, timeout=300, **win_kwargs)
    if test_proc.returncode != 0:
        print(f"❌ campaign_orchestrator.py 검증 실패 (exit {test_proc.returncode})")
        print("   먼저 campaign_orchestrator.py 단독으로 ▶ 실행해서 설정·LLM 연결 확인 후 재시도.")
    ...
    try:
        while True:
            ...
            print(f"\n[{ts}] 🤖 {loop}회차 캠페인 발행 (가동 {elapsed_h:.1f}시간) [최신 리스크: {risk:.1f}%]")
            _write_state("RUNNING", loop, start_ts, elapsed_h, ts, next_ts, risk, current_interval)
            
            try:
                subprocess.run([sys.executable, ORCHESTRATOR_PATH], check=False, **win_kwargs)
