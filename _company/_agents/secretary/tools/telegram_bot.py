...
SECRETARY_TOOL_JSON = os.path.join(HERE, "telegram_setup.json")
SECRETARY_CFG = os.path.join(HERE, "..", "config.md")
PLANNER_STATE_PATH = os.path.join(HERE, "planner_state.json")
TREND_SNIPER_PATH = os.path.abspath(os.path.join(HERE, "..", "..", "writer", "tools", "trend_sniper.py"))
SNIPER_REPORT_PATH = os.path.abspath(os.path.join(HERE, "..", "..", "writer", "tools", "trend_sniper_report.md"))
NAVER_WRITER_PATH = os.path.abspath(os.path.join(HERE, "..", "..", "writer", "tools", "naver_writer.py"))
NAVER_PUBLISHER_PATH = os.path.abspath(os.path.join(HERE, "..", "..", "writer", "tools", "naver_publisher.py"))
...
SEO_AGENT_PATH = os.path.abspath(os.path.join(HERE, "..", "..", "..", "scripts", "seo_agent.py"))
EMPATHY_PROFILER_PATH = os.path.abspath(os.path.join(HERE, "..", "..", "empathy_profiler.py"))
...
def _resolve_telegram():
    """비서 설정 -> Legacy MD 순서로 텔레그램 설정 로드."""
    token, chat = "", ""
...
def get_latest_visual_assets():
    """visual_guides 폴더에서 가장 최근에 저장된 실물 1:1 감성 엽서 이미지 경로를 반환합니다."""
    guides_dir = os.path.abspath(os.path.join(HERE, "..", "..", "designer", "tools", "visual_guides"))
    if not os.path.exists(guides_dir):
        return None
    try:
        ...
        postcards = [f for f in files if "postcard_" in f or "thumbnail_" in f or "card_news_" in f]
        if not postcards:
            return None
        return max(postcards, key=os.path.getmtime)
    except Exception:
        return None
...
def _resolve_api_url():
    """telegram_setup.json에서 REMOTE_API_URL 값을 로드합니다. 기본값은 http://127.0.0.1:8000 입니다."""
...
# 📱 14대 핵심 제어 이모지 단축 키보드 레이아웃 개편 ('마음을 묻다' 특화)
KEYBOARD = {
    "keyboard": [
        [{"text": "🎯 심리 트렌드"}, {"text": "👥 사연 감정 진단"}],
        [{"text": "📝 치유 에세이"}, {"text": "🎨 감성 비주얼 엽서"}],
        [{"text": "🎵 명상 BGM"}, {"text": "🚀 블로그 발행"}],
        [{"text": "🔍 SEO 최적화"}, {"text": "📊 경영 관제"}],
...
def handle_command(cmd, token, chat_id):
    ...
    if cmd in ("/help", "❓ 도움말 안내"):
        help_text = """🤖 [마음을 묻다 — AI 1인 기업 원격 관제 비서]

아래 채팅창의 14대 감성 버튼을 터치하거나 명령어를 전송하여 툴들을 원터치 조종할 수 있습니다:

🎯 [심리 트렌드] : 트렌드 스나이퍼(trend_sniper.py) 즉시 실행 및 최근 고민 트렌드 회신.
👥 [사연 감정 진단] : 공감 프로파일러(empathy_profiler.py) 즉시 실행 및 5대 고민 감정 스코어링 진단.
📝 [치유 에세이] : 감성 작가(naver_writer.py) 즉시 가동 및 마음을 어루만지는 힐링 에세이 집필.
🎨 [감성 비주얼 엽서] : 비주얼 디렉터(visual_director.py) 가동 및 1:1 힐링 엽서 PNG 이미지 실물 피딩.
🎵 [명상 BGM] : 사운드 엔진(music_generate.py) 가동 및 528Hz 치유 주파수 명상 음원 MP3 실물 피딩.
🚀 [블로그 발행] : 네이버 퍼블리셔(naver_publisher.py) 자율 발행 및 포스팅 링크 회신.
🔍 [SEO 최적화] : SEO 에이전트(seo_agent.py) 가동 및 후기(reviews.json) 기반 메타데이터 자율 갱신.
📢 [캠페인 일괄 실행] : 마케팅 파이프라인(트렌드->에세이/엽서/BGM 병렬 창작->블로그 포스팅) 자동 가동.
💬 [사장님 피드백] : 지시사항(decisions.md) 반영을 위한 피드백 실시간 피딩.
🧹 [RAG 메모리 다이어트] : decisions.md를 96% 초압축 및 이중 격리 아카이빙.
❓ [도움말 안내] : 현재 가이드라인 및 조종법 안내."""
        send_message(token, chat_id, help_text, reply_markup=KEYBOARD)

    elif cmd in ("/trend", "🎯 심리 트렌드"):
        send_message(token, chat_id, "📡 [에이전트 기동] 심리 고민 트렌드 스나이퍼(trend_sniper.py)를 즉시 실행합니다. 약 10~30초 소요됩니다...")
        last_size = os.path.getsize(SNIPER_REPORT_PATH) if os.path.exists(SNIPER_REPORT_PATH) else 0
        
        try:
            proc = subprocess.run([sys.executable, TREND_SNIPER_PATH], capture_output=True, encoding="utf-8", timeout=180, **win_kwargs)
            if proc.returncode == 0:
                report = get_new_report_section(SNIPER_REPORT_PATH, last_size)
                if not report:
                    report = "✅ 분석 완료! trend_sniper_report.md 파일을 확인하세요."
                send_message(token, chat_id, f"🎯 [심리 고민 트렌드 분석 완료]\n\n{report}", reply_markup=KEYBOARD)
            else:
    ...
    elif cmd in ("/diagnose", "👥 사연 감정 진단"):
        send_message(token, chat_id, "📡 [에이전트 기동] 자율 공감 프로파일러(empathy_profiler.py)를 즉시 가동합니다. 테스트 사연을 진단합니다...")
        
        try:
            # 예시 사연을 넣어 감정 프로파일러 기동
            sample_story = "요즘 회사에서 번아웃이 심하게 와서 아침에 눈뜨기가 너무 무섭고 외로워요."
            proc = subprocess.run([sys.executable, EMPATHY_PROFILER_PATH, sample_story], capture_output=True, encoding="utf-8", timeout=180, **win_kwargs)
            if proc.returncode == 0:
                result_json = proc.stdout.strip()
                try:
                    data = json.loads(result_json)
                    emotions = data.get("emotions", {})
                    diag_text = f"""👥 [자율 심리 감정 프로파일 진단 완료]

● 테스트 사연: "{sample_story}"

📊 고민 사연 5대 감정 인디케이터:
- 🔴 불안 스코어: {emotions.get('anxiety', 0)} / 10
- 🔵 무기력 스코어: {emotions.get('lethargy', 0)} / 10
- 🟢 자책 스코어: {emotions.get('self_blame', 0)} / 10
- 🟡 슬픔 스코어: {emotions.get('sadness', 0)} / 10
- 🟣 고독 스코어: {emotions.get('loneliness', 0)} / 10

🎯 통점 및 핵심 키워드: {', '.join(data.get('pain_points', []))}
🛡️ 방어기제 분석: {data.get('defense_mechanism', 'N/A')}
✍️ 오영범 마스터 처방 지침:
- {data.get('prescriptive_guideline', '섣부른 조언을 배제하고 깊이 공감하십시오.')}"""
                    send_message(token, chat_id, diag_text, reply_markup=KEYBOARD)
                except Exception:
                    send_message(token, chat_id, f"👥 [자율 감정 진단 결과]\n\n{result_json}", reply_markup=KEYBOARD)
            else:
                err_msg = proc.stderr.strip()[-300:] if proc.stderr else "상세 에러 내용 없음"
                send_message(token, chat_id, f"❌ 공감 프로파일러 가동 실패 (exit {proc.returncode}).\n에러 요약: {err_msg}", reply_markup=KEYBOARD)
        except Exception as e:
            send_message(token, chat_id, f"❌ 공감 프로파일러 기동 실패: {e}", reply_markup=KEYBOARD)

    elif cmd in ("/write_blog", "📝 치유 에세이"):
        send_message(token, chat_id, "📡 [에이전트 기동] 네이버 칼럼 감성 작가(naver_writer.py)를 즉시 가동합니다. 약 10~30초 소요됩니다...")
        ...
        try:
            ...
            if proc.returncode == 0:
                ...
                if not post:
                    post = "✅ 감성 에세이 생성 완료! naver_posts/ 폴더에서 마크다운 파일을 확인하십시오."
                send_message(token, chat_id, f"📝 [오영범 마스터 감성 치유 에세이 집필 완료]\n\n{post}", reply_markup=KEYBOARD)
            else:
                err_msg = proc.stderr.strip()[-300:] if proc.stderr else "상세 에러 내용 없음"
                send_message(token, chat_id, f"❌ 감성 작가 가동 실패 (exit {proc.returncode}).\n에러 요약: {err_msg}", reply_markup=KEYBOARD)
        except Exception as e:
            send_message(token, chat_id, f"❌ 감성 작가 실행 도중 장애 발생: {e}", reply_markup=KEYBOARD)

    elif cmd in ("/visual", "🎨 감성 비주얼 엽서"):
        send_message(token, chat_id, "📡 [에이전트 기동] 비주얼 디렉터(visual_director.py)를 가동해 엽서 렌더링을 즉시 시작합니다. 약 10~30초 소요됩니다...")
        ...
        try:
            ...
            if proc.returncode == 0:
                ...
                send_message(token, chat_id, f"🎨 [마음을 묻다 1:1 감성 엽서 기획 완료]\n\n{guide}", reply_markup=KEYBOARD)
                
                # 실물 엽서 이미지(postcard_*.png) 자동 전송
                latest_postcard = get_latest_visual_assets()
                if latest_postcard:
                    send_photo(token, chat_id, latest_postcard, "🎨 [마음을 묻다] 생성된 1:1 아날로그 감성 힐링 엽서 이미지입니다.")
                else:
                    send_message(token, chat_id, "⚠️ 생성된 실물 엽서 이미지를 찾을 수 없습니다.", reply_markup=KEYBOARD)
            else:
                err_msg = proc.stderr.strip()[-300:] if proc.stderr else "상세 에러 내용 없음"
    ...
    elif cmd in ("/publish_blog", "🚀 블로그 발행"):
        send_message(token, chat_id, "📡 [에이전트 기동] 네이버 자율 발행 어댑터(naver_publisher.py)를 기동합니다...")
        
        try:
            proc = subprocess.run([sys.executable, NAVER_PUBLISHER_PATH], capture_output=True, encoding="utf-8", timeout=240, **win_kwargs)
            if proc.returncode == 0:
                p_status, p_url = "simulated", ""
                try:
                    json_str = proc.stdout.strip().split("="*50)[-2].strip()
                    data = json.loads(json_str)
                    p_status = data.get("status", "simulated")
                    p_url = data.get("url", "")
                except Exception:
                    pass
                msg = f"🚀 [네이버 블로그 감성 에세이 자율 발행 완료]\n\n● 발행 상태: {p_status}\n🔗 포스팅 링크: {p_url or '시뮬레이션 모드'}"
                send_message(token, chat_id, msg, reply_markup=KEYBOARD)
            else:
                err_msg = proc.stderr.strip()[-300:] if proc.stderr else "상세 에러 내용 없음"
                send_message(token, chat_id, f"❌ 네이버 발행기 가동 실패 (exit {proc.returncode}).\n에러 요약: {err_msg}", reply_markup=KEYBOARD)
        except Exception as e:
            send_message(token, chat_id, f"❌ 네이버 발행기 가동 중 장애 발생: {e}", reply_markup=KEYBOARD)

    elif cmd in ("/music", "🎵 명상 BGM"):
        send_message(token, chat_id, "📡 [에이전트 기동] 명상 사운드 엔진(music_generate.py)을 가동해 528Hz 치유 음원을 조제합니다...")
        
        try:
            proc = subprocess.run([sys.executable, MUSIC_GENERATOR_PATH], capture_output=True, encoding="utf-8", timeout=240, **win_kwargs)
            if proc.returncode == 0:
                send_message(token, chat_id, "✅ 명상 BGM 작곡 완료! 528Hz 치유 솔페지오 주파수 음원을 실물 피딩 전송합니다.", reply_markup=KEYBOARD)
                latest_audio = get_latest_bgm_assets()
                if latest_audio:
                    send_audio(token, chat_id, latest_audio, "🎵 [마음을 묻다 528Hz 치유 명상 BGM]")
                else:
                    send_message(token, chat_id, "⚠️ 생성된 치유 명상 오디오 파일을 찾을 수 없습니다.", reply_markup=KEYBOARD)
            else:
                err_msg = proc.stderr.strip()[-300:] if proc.stderr else "상세 에러 내용 없음"
                send_message(token, chat_id, f"❌ 명상 사운드 엔진 가동 실패 (exit {proc.returncode}).\n에러 요약: {err_msg}", reply_markup=KEYBOARD)
        except Exception as e:
            send_message(token, chat_id, f"❌ 명상 사운드 엔진 가동 중 장애 발생: {e}", reply_markup=KEYBOARD)

    elif cmd in ("/seo", "🔍 SEO 최적화"):
        send_message(token, chat_id, "📡 [에이전트 기동] SEO 최적화 및 메타데이터 자율 갱신 에이전트(seo_agent.py)를 실행합니다...")
        
        try:
            proc = subprocess.run([sys.executable, SEO_AGENT_PATH], capture_output=True, encoding="utf-8", timeout=240, **win_kwargs)
            if proc.returncode == 0:
                send_message(token, chat_id, "🔍 [자율 SEO 마케터 업데이트 완료]\n\n최근 치유 후기(reviews.json) 데이터의 통증 단어(예: 불안, 번아웃, 우울)를 자율 분석하여 Next.js 프론트엔드의 메타데이터(layout.tsx) description을 완벽하게 업데이트하고 검색엔진 노출을 최적화시켰습니다!", reply_markup=KEYBOARD)
            else:
                err_msg = proc.stderr.strip()[-300:] if proc.stderr else "상세 에러 내용 없음"
                send_message(token, chat_id, f"❌ SEO 에이전트 가동 실패 (exit {proc.returncode}).\n에러 요약: {err_msg}", reply_markup=KEYBOARD)
        except Exception as e:
            send_message(token, chat_id, f"❌ SEO 에이전트 실행 도중 장애 발생: {e}", reply_markup=KEYBOARD)

    elif cmd in ("/campaign", "📢 캠페인 일괄 실행"):
        send_message(token, chat_id, "⚡ [Ryzen 9 병렬 가동] 마음을 묻다 자율 오케스트레이터(campaign_orchestrator.py) 연쇄 기동 중... 약 15~25초 소요됩니다.")
        ...
        try:
            ...
            if proc.returncode == 0:
                ...
                summary_msg = f"""📢 [⚡ 마음을 묻다 통합 치유 캠페인 완수!]

사장님, '마음을 묻다'의 모든 치유 마케팅 연쇄가 자율 병렬 기동되어 성공적으로 끝났습니다!

📅 캠페인 회차: campaign_{campaign_id}
📂 포트폴리오: _company/marketing_history/campaign_{campaign_id}/
⚡ 실행 소요 시간: {data.get('elapsed_seconds', 'N/A')}초 (병렬 처리 고성능 완착)

🛡️ [마음을 묻다 에이전트 병렬 완료 현황]
● 심리 고민 트렌드 스캔: {data['steps'].get('trend_sniper', 'N/A')}
● 오영범 마스터 치유 에세이: {data['steps'].get('naver_writer', 'N/A')}
● 1:1 감성 엽서 비주얼: {data['steps'].get('visual_director', 'N/A')}
● 528Hz 치유 명상 BGM: {data['steps'].get('music_generator', 'N/A')}

🚀 [채널 자율 배포 및 상위 노출 현황]
● 네이버 블로그: {data['steps']['naver_publish'].get('status', 'N/A')}
🔗 링크: {data['steps']['naver_publish'].get('url', '시뮬레이션 모드')}

🤖 실시간 트래픽, 감사 로그는 SQLite DB(marketing.db, gateway_audit.db)에 적재되어 안전하게 보존되었습니다."""
                send_message(token, chat_id, summary_msg, reply_markup=KEYBOARD)
                
                # 캠페인 엽서 실물 이미지와 명상 BGM 자동 전송
                if campaign_id != "N/A":
                    postcard_path = os.path.abspath(os.path.join(HERE, "..", "..", "..", "marketing_history", f"campaign_{campaign_id}", "03_visual_guide.md")) # 텍스트 가이드 외에 이미지 매칭
                    bgm_path = os.path.abspath(os.path.join(HERE, "..", "..", "..", "marketing_history", f"campaign_{campaign_id}", "05_signature_bgm.mp3"))
                    
                    # 엽서 PNG 이미지 탐색 후 발송
                    guides_dir = os.path.abspath(os.path.join(HERE, "..", "..", "..", "marketing_history", f"campaign_{campaign_id}"))
                    if os.path.exists(guides_dir):
                        pngs = [os.path.join(guides_dir, f) for f in os.listdir(guides_dir) if f.endswith(".png")]
                        for p in pngs:
                            send_photo(token, chat_id, p, "🎨 [마음을 묻다] 이번 캠페인 자율 합성 실물 엽서 이미지입니다.")
                    
                    if os.path.exists(bgm_path):
                        send_audio(token, chat_id, bgm_path, "🎵 [마음을 묻다] 이번 캠페인 전용 시그니처 명상 BGM 테마곡입니다.")
            else:
                err_msg = proc.stderr.strip()[-300:] if proc.stderr else "상세 에러 내용 없음"
